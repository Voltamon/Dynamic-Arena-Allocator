...
int* a = arena->alloc(arena, sizeof(int) * 10);     // Pointer math
char* b = arena->alloc(arena, 100);                  // Pointer math
double* c = arena->alloc(arena, sizeof(double) * 5); // Pointer math

arena->destroy(arena);  // Free everything at once
```
...
```c
int* numbers = (int*)arena->alloc(arena, sizeof(int) * 100);
...
int main(void) {
    ...
    int* data = (int*)arena->alloc(arena, sizeof(int) * 100);
    for (int i = 0; i < 100; i++) {
        data[i] = i * i;
    }
...
```c
void* arena->alloc(Arena* self, size_t size);
```
Allocates `size` bytes from the arena. Returns pointer or NULL on failure.

```c
void* arena->alloc_aligned(Arena* self, size_t size, size_t alignment);
```
Allocates `size` bytes aligned to `alignment` boundary (must be power of 2).

```c
void* arena->realloc(Arena* self, void* ptr, size_t old_size, size_t new_size);
```
...
```c
void arena->print_stats(Arena* self);
```
...
Save your position, do temporary work, then restore:
...
State 3: Restore to Checkpoint
┌─────────────────────────────────────┐
│ ┌─────────┬─────────────────────────┐ │
│ │ Used    │  Available Again!       │ │
│ └─────────┴─────────────────────────┘ │
│ offset: 500 ← restored              │
└─────────────────────────────────────┘
```
...
while (game_running) {
    Entity* entities = arena->alloc(frame_arena, sizeof(Entity) * count);
    Particle* particles = arena->alloc(frame_arena, sizeof(Particle) * p_count);
...
void process_data(void) {
    size_t checkpoint = scratch->get_mark(scratch);

    char* temp_buffer = scratch->alloc(scratch, 2048);
    int* temp_array = scratch->alloc(scratch, sizeof(int) * 1000);

    scratch->reset_to_mark(scratch, checkpoint);
}
```
...
string_arena->destroy(string_arena);
```
...
float* vectors = (float*)simd_arena->alloc_aligned(simd_arena,
                                                    sizeof(float) * 1024,
                                                    16);

simd_arena->destroy(simd_arena);
```
...
alloc(40)   ──→  [A|B|C|Empty]  Just pointer math
alloc(100)  ──→  [A|B|C|Empty]  Just pointer math
alloc(200)  ──→  [A|B|C|Empty]  Just pointer math

destroy()   ──→  Free all        One call
```
...
void process_data(Arena* arena) {
    int* data = arena->alloc(arena, sizeof(int) * 100);
}
```
...
void thread_cleanup(void) {
    thread_arena->destroy(thread_arena);
}
```
...
2. **Use checkpoints for nested operations**
   ```c
   size_t mark = arena->get_mark(arena);
   arena->reset_to_mark(arena, mark);
   ```
...
5. **Use aligned allocations for SIMD**
   ```c
   float* data = arena->alloc_aligned(arena, size, 16);
   ```
